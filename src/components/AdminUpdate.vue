...
onMounted(async () => {
  ...
  const game = await axios.get(`http://localhost:8080/api/v1/videogames/${id}`);
  gameId.value = id;
  name.value = game.data.name;
  releaseDate.value = game.data.releaseDate;
  company.value = game.data.company;
  platform.value = game.data.platform
  image.value = game.data.image
  
  for (let i = 0; i < genres.value.length; i++) {
    genres.value[i].checked = game.data.genres.includes(response.data[i].name)
  }
...
async function handleUpdate(event) {
  ...
  axios.put(
    `http://localhost:8080/api/v1/videogames/${gameId.value}`,
    {
      name: name.value,
